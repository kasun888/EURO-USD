"""Main orchestrator for Fiber Scalp v1.5 — EUR/USD M5 Scalper
...
State isolation:
  ...
  Signal cache, ops state, and cooldown state are per-pair files, e.g.:
    score_cache_eurusd.json / ops_state_eurusd.json / runtime_pair_eurusd.json
"""
...
from circuit import is_win_stop_active, is_circuit_breaker_active, record_win, record_sl, reset_consec_losses
...
def _pair_key(instrument: str) -> str:
    """'EUR_USD' → 'eurusd'  (used as file-name suffix)."""
...
def _pair_state_file(base: Path, instrument: str) -> Path:
    ...
    e.g. score_cache.json → score_cache_eurusd.json
    """
...
def _pip_dp(pip: float) -> int:
    ...
    if pip <= 0.01:   return 3   # JPY pairs — Fiber Scalp v1.5
    return 2
...
def validate_settings(settings: dict) -> dict:
    required = ["pairs"]  # Fiber Scalp v1.5: pair_sl_tp fixed pips used exclusively
    ...
    settings.setdefault("weekly_export_hour_sgt",      8)
    settings.setdefault("weekly_export_minute_sgt",    5)
    ...
    settings.setdefault("pair_sl_tp", {
        "EUR_USD": {"sl_pips": 18, "tp_pips": 30, "pip_value_usd": 10.0, "be_trigger_pips": 20},
    })
...
def calculate_units_from_position(position_usd: int, sl_usd: float) -> float:
    """units = position_usd / sl_price_distance. Exact USD risk for EUR/USD."""
...
def _guard_phase(db, run_id, settings, alert, history, now_sgt, today, demo,
    ...
    log.info("=== %s | %s | %s SGT ===",
             settings.get("bot_name", "Fiber Scalp v1.5"), instrument,
